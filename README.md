...
When you want to remove part of a string matching a regex, don't use `regex_replace!` with an empty string, but rather the [`regex_remove!`](https://docs.rs/lazy-regex/latest/lazy_regex/macro.regex_remove.html) macro which is more efficient and returns a borrowed slice when possible.
